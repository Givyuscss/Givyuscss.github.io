...
subtitle:   " \"Multi-View-Clustering 详解\""
...
\end{equation}
$$\gamma$$和$$\sigma^2$$分别表示高斯分布的均值和方差
...
对于多视角的CMMs，每个$$x_i$$都有m个视角，即$$x_i=\{x^1_i,x^2_i,…,x^m_i\}$$则每个视角的混合分布为
$$
\begin{equation}
...
$$
\begin{equation}
q^{(t+1)}=\frac{q^{(t)}_j}{M}\sum^m_{v=1}\sum^N_{i=1}\frac{\hat{P}f^v_j(x^v_i)}{\sum^N_{j=1}q^{(t)}_{j}f^v_{j}f^v_{j}(x^v_i)}
\end{equation}
$$
$$q_j$$表示第j个数据作为范例的置信度
...
$$
\begin{cases} 
&\mathop{min}\limits_{Z^{(s)},s=1,2,…,m}\sum^m_{s=1}\|X^{(s)}-X^{(s)}Z^{(s)}\|^2_F+\alpha\sum^m_{s=1}\|Z^{(s)}\|_1+\beta\sum_{1\le s\le t}\|Z^{(s)}-Z^{(t)}\|_1 \\&s.t. \ diag(Z^{(s)})=1,\forall s\in \{1,2,…,m\} 
\end{cases}
$$
